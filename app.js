...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  // api.ai client
  const apiClient = new ApiAi.ApiAiClient({accessToken: '8c5a425daf2748378905b08fed73b012'});
  
  // Initial feedback message.
  addBotItem("Hi! I’m Joi. Tap the microphone and start talking to me.");
  ...
  recognition.onresult = function(ev) {
	...
	//window.open('http://testapp.talkadoc.com/encounters/view/46259', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
    ...
    function handleResponse(serverResponse) {
	  ...
	  //var speech = null;
	 //for (var prop in serverResponse["result"]["fulfillment"]["messages"][0]) {
    // 	  addBotItem(prop);
	// }
      const speech = serverResponse["result"]["fulfillment"]["messages"][0]["speech"];
      var msg = new SpeechSynthesisUtterance(speech);
	  var action = '';
	  addBotItem(speech);
	  if (typeof serverResponse["result"]["fulfillment"]["messages"][1] !== 'undefined') {
		action = serverResponse["result"]["fulfillment"]["messages"][1]["speech"];
	  }
      ...
      switch(action) {
		case "casesheet":
			//text = "Opening Case Sheet!";
			window.open('http://testapp.talkadoc.com/encounters/view/46259', '_blank', 'location=no,top=50,left=50,height=600,width=570,scrollbars=yes,status=yes');
			break;
		case "prescription":
			//text = "I am not a fan of prescription.";
			window.open('http://testapp.talkadoc.com/prescriptions/pview/55898', '_blank', 'location=no,top=50,left=50,height=600,width=570,scrollbars=yes,status=yes');
			break;
		case "patientinfo":
			//text = "I am not a fan of prescription.";
			window.open('http://testapp.talkadoc.com/patients/view/35606', '_blank', 'location=no,top=50,left=50,height=600,width=570,scrollbars=yes,status=yes');
			break;
		default:
